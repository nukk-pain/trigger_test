...
export class OpenRouterConfig {
  constructor() {
    ...
    async makeRequest(messages, systemPrompt = '') {
        ...
        // 사용량 제한 확인
        const canShowClientAllowance = window.usageTracker.canShowClientRequestAllowance
            ? window.usageTracker.canShowClientRequestAllowance(window.envLoader)
            : window.usageTracker.canMakeRequest(window.envLoader);

        if (!canShowClientAllowance) {
